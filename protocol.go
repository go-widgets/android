...
// MapTouch maps one pointer sample to toolkit events. It mirrors the wasmbox
// and X11 mappings: a press is a click, a move with the finger down is a drag.
// A touch screen has no hover, so a move with no finger down cannot occur and
// is mapped to a plain move rather than dropped, keeping a synthetic host
// (a test, a replay) honest.
func MapTouch(t Touch, held bool) []toolkit.Event {
	switch t.Action {
	case TouchDown:
		return []toolkit.Event{{Kind: toolkit.EventClick, X: t.X, Y: t.Y}}
	case TouchUp:
		return []toolkit.Event{{Kind: toolkit.EventMouseUp, X: t.X, Y: t.Y}}
	case TouchMove:
		kind := toolkit.EventMouseMove
		if held {
			kind = toolkit.EventMouseDrag
		}
		return []toolkit.Event{{Kind: kind, X: t.X, Y: t.Y}}
	default:
		return nil
	}
}
